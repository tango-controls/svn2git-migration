...
foreach my $ticket (@tickets) {
	...
	# Here we assume the user map contains all the people having tickets assigned
	...
	if ($proxy) {
		$command = "curl -x $proxy -X POST -H \"Authorization: token $GITHUB_TOKEN\" -H \"Accept: $ACCEPT\" -d \@$jsfile https://api.github.com/repos/$REPO/import/issues\n";
	}
    print $command;
    if ($dry_run) {
...
sub usage {
    ...
    <<EOM;
$sn [-h] [-u USERMAP] [-c COLLABINFO] [-r REPO] [-t OAUTH_TOKEN] [-a USERNAME] [-l LABEL]* [-s SF_TRACKER] [-x PROXY] [--dry-run] TICKETS-JSON-FILE
...
Author: [Chris Mungall](https://github.com/cmungall), [Reynald Bourtembourg](https://github.com/bourtemb)
