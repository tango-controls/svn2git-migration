...
def usage():
	...
	print 'not containing the given labels nor having the specified ticket number and save them into the output json file'
...
def extract_tickets_to_json_file(tickets,tickets_num_set,outputjsonfilename):
	...
	for ticket in tickets:
		if ticket["ticket_num"] not in tickets_num_set:
...
def main(argv):
	...
	if len(labels) == 0 and len(tickets_num_set) == 0 and len(categories) == 0:
